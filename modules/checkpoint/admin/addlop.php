...
if ($nv_Request->get_int ( 'save', 'post' )) {
	...
	if (! empty ( $id )) {
		$result = $db->sql_query ( "UPDATE `" . NV_PREFIXLANG . "_" . $module_data . "_lop` SET `tenlop`=" . $db->dbescape ( $tenlop ). ",`gvid`=" . $db->dbescape ( $gvid ). "  WHERE lopid=" . $id . "" );
		if ($result) {
		...
		if (! $alreadylopid) {
			$result = $db->sql_query ( "INSERT INTO `" . NV_PREFIXLANG . "_" . $module_data . "_lop` (`lopid`, `tenlop`,`gvid`) VALUES (" . $db->dbescape ( $lopid ) . ", " . $db->dbescape ( $tenlop ) . ", " . $db->dbescape ( $gvid ) . ")" );
			if ($result) {
